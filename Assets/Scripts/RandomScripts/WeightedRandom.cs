...
namespace SimpleRandomizer
{
    public class WeightedRandom
    {
        ...
        public int WeightedRandomIndex(List<float> weightList)
        {
            totalSum = 0;
        ...
        public float WeightedRandomValue(List<float> weightList)
        {
            totalSum = 0;
            var sortedWeight = weightList.Select((item, index) => new {item, index}).OrderBy(o => o.item).ToDictionary(o => o.index, o => o.item);
        ...
        KeyValuePair<TKey, float> CompareResult<TKey>(Dictionary<TKey, float> sortedWeight)
        {
            foreach(var weight in sortedWeight)
            {
                totalSum += weight.Value;
